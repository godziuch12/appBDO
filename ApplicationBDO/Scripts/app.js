		...
		app.SmartActions = function(){
			...
			var smartActions = {
			    ...
			    resetWidgets: function($this){
					...
					$.SmartMessageBox({
                        title: "<i class='fa fa-refresh' style='color:green'></i> Odświerz i wyczyść pamięć podręczną",
                        content: $this.data('reset-msg') || "Czy chcesz RESETOWAĆ wszystkie zapisane widżety i wyczyścić pamięć podręczną?",
						buttons : '[Nie][Tak]'
					}, function(ButtonPressed) {
...
function drawBreadCrumb(opt_breadCrumbs) {
    ...
    var url = location.href.split('/').splice(3);
		bread_crumb.empty(), 
		bread_crumb.append($("<li>Strona głowna</li>")), a.each(function() {
    ...
    if (url[1] == "edit") {
        bread_crumb.append($("<li>Edycja</li>"))
    }
    if (url[1] == "delete") {
        bread_crumb.append($("<li>Usunięcie</li>"))
    }
