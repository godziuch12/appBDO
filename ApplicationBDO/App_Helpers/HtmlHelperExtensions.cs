...
namespace ApplicationBDO.App_Helpers
{
    public static class HtmlHelperExtensions
    {
        ...
        /// <param name="controller">parametr okresla nazwe kontrolera przynaleznego.</param>
        /// <returns>A HtmlString containing the given attribute value; otherwise an empty string.</returns>
        public static IHtmlString RouteIf(this HtmlHelper helper, string value, string attribute, string controller = null, List<string> parentControler = null)
        {
            var currentController =
                (helper.ViewContext.RequestContext.RouteData.Values["controller"] ?? string.Empty).ToString().UnDash();
            var currentAction =
                (helper.ViewContext.RequestContext.RouteData.Values["action"] ?? string.Empty).ToString().UnDash().ToLower();
            ...
            {
                foreach (string par in parentControler)
                {
                    if(currentController == par)
                    {
                        action = attribute;
                    }
                }
            ...
            //jesli nie jest przekazany parametr conttroler i currentController jest rowny nazwie kontrolera
            if (currentController == value)
            {
                action = attribute;
            }
            //jesli kontroler nadrzedny jest okreslony przyrownuje do kontrolera pobranego z danej akcji 
            if(currentController == controller && currentAction == value)
            {
                action = attribute;
            }
