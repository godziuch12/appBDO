...
using ApplicationBDO.ViewModels;
...
namespace ApplicationBDO.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Logout()
        {
            ...
            return RedirectToAction("Index", "IdentityCards");
        }
        ...
        public ActionResult Register()
        {
            ViewBag.Name = new SelectList(context.Roles
                .ToList(), "Name", "Name");
            var viewModel = new RegisterViewModel() { RolesList = new List<SelectListItem>() };
            var roles = RoleManager.Roles.OrderBy(r => r.Name);
            foreach (var role in roles)
            {
                var roleVm = new SelectListItem()
                {
                    Text = role.Name,
                    Selected = false,
                };
                viewModel.RolesList.Add(roleVm);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var rolesIsSelected = model.RolesList.Where(r => r.Selected).ToList();
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);

                    await this.UserManager.AddToRolesAsync(user.Id, rolesIsSelected.Select(r => r.Text).ToArray());
